...
import { ContractorDeliveryModal } from '../components/modals/ContractorDeliveryModal';
...
export const ContractorDetails = () => {
  ...
  const loadDetails = useCallback(async () => {
    try {
      setLoading(true);
      const res = await apiGet(`/contractors/${id}`, token);
      setData(res);
    } catch (err) {
      Swal.fire('خطأ', err.message || 'فشل تحميل تفاصيل المقاول', 'error');
    } finally {
      setLoading(false);
    }
  }, [id, token]);
  ...
  const delFilters = useTableFilters({
    data: deliveriesData,
    searchFields: ['material', 'voucher', 'client_name', 'crusher_name', 'driver_name'],
  ...
  const handlePaymentSubmit = async (formData) => {
    ...
    try {
      if (selectedItem) {
        await apiPut(`/contractors/${id}/payments/${selectedItem.id || selectedItem._id}`, payload, token);
        Swal.fire('تم بنجاح', 'تم تحديث الدفعة بنجاح', 'success');
      } else {
        await apiPost(`/contractors/${id}/payments`, payload, token);
  ...
  const handleAdjustmentSubmit = async (formData) => {
    try {
      if (selectedItem) {
        await apiPut(`/contractors/${id}/adjustments/${selectedItem.id || selectedItem._id}`, formData, token);
        Swal.fire('تم بنجاح', 'تم تعديل التسوية بنجاح', 'success');
      } else {
        await apiPost(`/contractors/${id}/adjustments`, formData, token);
    ...
    if (result.isConfirmed) {
      try {
        if (type === 'delivery') {
          ...
          await apiDelete(`/contractors/${id}/payments/${itemId}`, token);
        } else {
          await apiDelete(`/contractors/${id}/adjustments/${itemId}`, token);
        }
  ...
  const generateReport = (type) => {
    ...
    if (type === 'deliveries') {
      ...
      window.open(`${baseUrl}/contractors/${id}/reports/deliveries?from=${repFrom}&to=${repTo}`, '_blank');
    } else {
      let url = `${baseUrl}/contractors/${id}/reports/statement`;
      if (useStmtDates) {
        if (!stmtFrom || !stmtTo) {
  ...
  if (loading) return <Loader text="جاري تحميل تفاصيل المقاول..." />;
  if (!data) return <div className="error-box">حدث خطأ في تحميل التفاصيل</div>;

  const { contractor, totals } = data;
  ...
  return (
    <div className="contractor-details-page">
      {/* Header */}
      <div className="page-header">
        <div>
          <h1 className="page-title">{contractor.name}</h1>
          <p className="page-subtitle">دفتر الأستاذ والعمليات المالية للمقاول</p>
        </div>
      ...
      <div className="summary-grid">
        <div className="summary-card">
          ...
          <span className="summary-card-val">{formatCurrency(totals.openingBalance)}</span>
        </div>
        <div className="summary-card text-danger">
          <span className="summary-card-lbl">إجمالي مستحقات المشاوير</span>
          <span className="summary-card-val">{formatCurrency(totals.totalTrips)}</span>
        </div>
        <div className="summary-card text-success">
          <span className="summary-card-lbl">إجمالي المدفوعات</span>
          <span className="summary-card-val">{formatCurrency(totals.totalPayments)}</span>
        </div>
        <div className="summary-card">
          <span className="summary-card-lbl">إجمالي التعديلات</span>
          <span className={`summary-card-val ${totals.totalAdjustments >= 0 ? 'text-danger' : 'text-success'}`}>
            {formatCurrency(Math.abs(totals.totalAdjustments))}
          </span>
        </div>
      ...
      <div className="tabs-container">
        <button className={`tab-btn ${tab === 'deliveries' ? 'active' : ''}`} onClick={() => setTab('deliveries')}>المشاوير / التسليمات</button>
        ...
        {tab === 'deliveries' && (
          <div>
            <div className="table-filters-container" style={{ marginBottom: '1rem', gap: '0.5rem' }}>
              <input
                type="text"
                placeholder="بحث في العميل / الكسارة / المادة..."
            ...
            {delFilters.filteredData.length === 0 ? (
              <div className="no-records-text" style={{ padding: '2rem', textAlign: 'center', color: '#888' }}>لا توجد سجلات تسليم مطابقة.</div>
            ) : (
                  ...
                  <tbody>
                    {delFilters.filteredData.map(d => (
                      <tr key={d.id || d._id}>
                        ...
                        <td>{formatCurrency(d.contractor_total_charge || d.contractor_charge || 0)}</td>
                        <td>
        ...
        {tab === 'payments' && (
          <div>
            ...
            {payFilters.filteredData.length === 0 ? (
              <div className="no-records-text" style={{ padding: '2rem', textAlign: 'center', color: '#888' }}>لا توجد دفعات مالية مسجلة.</div>
            ) : (
              <div className="table-responsive">
                <table className="data-table">
                  <thead>
                    ...
                    {adjFilters.filteredData.map(a => (
                      <tr key={a.id || a._id}>
                        ...
                        <td className={a.amount >= 0 ? 'text-danger' : 'text-success'}>
                          {a.amount >= 0 ? 'إضافة (مستحق للمقاول)' : 'خصم (مستحق لنا)'}
                        </td>
                        <td>{a.reason || '-'}</td>
                        <td>
                          <div className="row-actions">
